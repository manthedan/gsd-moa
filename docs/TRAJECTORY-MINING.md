# Trajectory Mining: Droid vs omp on torch and a hard file-generation task

2026-07-04. Follow-up to the S2 four-way read (`docs/TERMINAL-BENCH-RESULTS.md`) and F0 forensics. Compares Factory Droid's passing runs against our omp single-arm failures on the two tasks where Droid broke our "capability floor": `torch-tensor-parallelism` (Droid 2/3, us 0/3) and a hard Terminal-Bench file-generation task (Droid 1/3, us 0/3). Same GPT-5.5, same CLIProxy, same task container image; the only variable is the agent harness and its tools.

> Scope note (for future sessions): the second task is treated here as an opaque Terminal-Bench coding/file-output benchmark. This document is harness forensics only: whether each agent's code-execution tools could run Python and shell commands inside the container. Domain details are intentionally omitted from this handoff-facing writeup.

## Headline: our harness could not execute Python in the task container; Droid's could

Across all three of our torch failures and all three hard-file-task failures, the same defect appears:
...
So our agent has no way to run or test the code it writes. Droid's `Execute` tool runs `python3 - <<'PY' … PY` heredocs natively and successfully (it invoked python3 2–3× per torch trial and repeatedly on the hard file-generation trial).

**Root cause (corrected 2026-07-04, verified empirically): the images ship no Python at all — the gap is in our adapter's install phase, not in the omp tools.** Direct probes of the torch image and the hard-file-task image on yukon found **no python3 binary anywhere in either image** (no `/usr/bin/python3`, empty `/usr/local/bin`, no conda/venv, nothing on a full filesystem search). The earlier read of this data — "PATH/environment defect in how our tools spawn processes; the interpreter is present" — was wrong on both counts:
...
## Hard file-generation task — same root cause, plus a second-order tooling gap

The task: write a required output file and validate it with the task-provided command-line checker. Droid's passing run and ours diverge the same way at the tool layer:

- **Droid** (`nHVdA3a`, pass): `python3` works, so it parses the inputs, installs the checker package cleanly, enumerates candidates, validates them end-to-end in Python via `subprocess`, writes the file, and re-validates. It also spawned a **sub-worker** (its `Task` subagent) for independent output checking — a parallel-verification pattern our harness never used.
- **Ours** (`6LcMYnn`, `AXMrJDj`, `WPNWsu9`, all fail): `eval` py unavailable → fall back to **JavaScript** (`require('fs')`) for input parsing — workable but clumsy, and it strands the agent because the validation tool is a CLI it wanted to drive from Python. All three mention the checker repeatedly and attempt the package install once, but flail on wiring CLI validation together from the JS/bash fallback and never close the loop the way Droid did in Python.
...
2. **Give the agent a reliable "install a CLI tool then drive it" recipe** (apt-get + subprocess) — Droid leaned on it for the hard file-generation task; ours didn't.
